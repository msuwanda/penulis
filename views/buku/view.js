...
function Views() {
  ...
  R.useEffect(() => {
    const data = { appkey: getKey(), pages: `${count}`, search: [{ name: "ns_judul", value: `${Search}` }, { name: "ns_penulis", value: `${Search2}` }, { name: "ns_audience", value: `${Search3}` }] };
  ...
  const items = Datas.map((data) =>
    <tr key={data.ns_id} className="transition-table">
      ...
      <td><ul className="pageaction">
        <button className="page-link-prev" onClick={() => window.location.href = '/600100#' + `${data.id}` }>Rinci</button>
          </ul>
      </td>
    </tr>
